...
Este é um micro framework para estudo do protocolo HTTP e Arquitetura de Software.
...
define('DB_PASS', "");
```
Importa o script **articles.sql** para seu banco de dados criado anteriormente.

Por fim executar o comando para inicialização do servidor local:
...
Este framework é apenas uma estrutura básica, portanto em trabalhos futuros é sugerido a implementação de algumas funcionalionalidades importantes no desenvolvimento de aplicaçõe web. Sento elas, Upload de arquivos, Validação de dados da requisição, Sessões, Cookies, Cache manipulação de imagens, etc. Além disso, pode ser implementado um novo container de execução, no qual permitirá a injeção de dependências nas classes. Isso faria com que o acoplamento diminuíesse consideravelmente e aumentasse a escalabilidade das aplicações desenvolvidas por esta ferramenta.  
